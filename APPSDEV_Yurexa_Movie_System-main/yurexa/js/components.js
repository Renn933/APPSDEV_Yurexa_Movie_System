...
function viewBrowse() {
  ...
  const filterButtons = ["All", ...GENRES].map(genre => `
    <button class="chip ${genre === "All" ? (currentFilter.length === 0 ? "chip--active" : "") : (Store.isFilterActive(genre) ? "chip--active" : "")}" data-action="filter-genre" data-genre="${genre}">
      ${genre}
    </button>
  `).join("");
  ...
  return `
    ...
    <section class="browse-filters">
      ${filterButtons}
      ${hasFilters ? `<button class="chip chip--clear" data-action="clear-filters"><i class="fas fa-times"></i> Clear Filters</button>` : ""}
    </section>
