...
// Handle all clicks on data-action and data-nav elements
function handleActionClick(e) {
  // Handle nav links (data-nav)
  const navTarget = e.target.closest("[data-nav]");
  if (navTarget) {
    e.preventDefault();
    const navName = navTarget.getAttribute("data-nav");
    if (Router.routes[navName]) {
      Router.navigate(navName);
    }
    return;
  }
  ...
  switch (action) {
    ...
    case "go-checkout":
      ...
      if (Store.getCartCount() === 0) {
        showToast("Your cart is empty. Add movies first.", "error");
        Router.navigate("browse");
      } else {
