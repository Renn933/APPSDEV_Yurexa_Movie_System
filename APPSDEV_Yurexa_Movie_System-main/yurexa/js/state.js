...
const Store = {
  ...
  init() {
    try {
      ...
      this.state.filter = JSON.parse(localStorage.getItem(this.keys.filter)) || [];
  ...
  // --- Filter/Sort methods ---
  // Toggle a genre in the filter stack (multiple genres can be active)
  setFilter(genre) {
    if (genre === "All") {
      this.state.filter = [];
    } else {
      const index = this.state.filter.indexOf(genre);
      if (index > -1) {
        this.state.filter.splice(index, 1);
      } else {
        this.state.filter.push(genre);
      }
    }
    this.save("filter");
  },
  ...
  isFilterActive(genre) {
    return this.state.filter.includes(genre);
  },

  // Check if any filters are active
  hasActiveFilters() {
    return this.state.filter.length > 0 || this.state.search.trim() !== "" || this.state.sort !== "featured";
  },
  ...
  getFilteredMovies() {
    ...
    // Genre filter (stacking — multiple genres)
    if (this.state.filter.length > 0) {
      movies = movies.filter(m => this.state.filter.includes(m.genre));
    }
