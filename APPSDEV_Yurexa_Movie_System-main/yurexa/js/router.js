...
const Router = {
  ...
  // Internal navigation history stack (for in-app back navigation)
  history: [],
  ...
  navigate(name, params = {}, options = {}) {
    // Push current route to history stack (if not already there)
    if (!options.skipHistory) {
      const currentKey = this.getRouteKey(this.current.name, this.current.params);
      const newKey = this.getRouteKey(name, params);
  ...
  // Get a unique key for a route (for dedup)
  getRouteKey(name, params = {}) {
    return name + (params.id ? `/${params.id}` : "");
  },

  // Go back to the previous in-app route
  goBack() {
    if (this.history.length > 0) {
      const prev = this.history.pop();
      this.navigate(prev.name, prev.params, { skipHistory: true });
    } else {
      this.navigate("home", {}, { skipHistory: true });
    }
  },
